    ...
    for( k=0; k<IMM_SIZE*(size+1)/TARGET; k++ ){
        ...
        for( i=START; i < END; i++ ){ //i is not reinizilized so that if i exit on VAL == TARGET i dont reenter
            for( j = 0; j < 256; j++ ){
                ...
                mask_x4 = aie::eq( x3, compare_x );
                mask_y4 = aie::eq( y3, compare_y );
                mask4 = mask_x4 & mask_y4 ;
                mask_x4 = aie::eq( x4, compare_x );
                mask_y4 = aie::eq( y4, compare_y );
                mask4 = mask_x4 & mask_y4 ;

                red = (int) mask1.count() + (int) mask2.count() + (int) mask4.count() + (int) mask4.count();
                val += red;
            ...
            hist = (int) mask_x1.count() + (int) mask_x2.count() + (int) mask_x4.count() + (int) mask_x4.count();
            if( hist ){
                writeincr( output, -2);
